...
class SSBDataset(Dataset):
    ...
    def _add_videos(self):
        self.arm_flapping_videos = [sorted(glob.glob(j + '/*.png'))[:20] for j in sorted(glob.glob(self.video_path+'ArmFlapping/*'))]
        self.head_banging_videos = [sorted(glob.glob(j + '/*.png'))[:20] for j in sorted(glob.glob(self.video_path+'HeadBanging/*'))]
        self.spinning_videos = [sorted(glob.glob(j + '/*.png'))[:20] for j in sorted(glob.glob(self.video_path+'Spinning/*'))]
